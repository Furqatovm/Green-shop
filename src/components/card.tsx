  ...
  return (
          <div>
                ...
                <div className={`gap-2 transform transition-transform duration-300 ease-in-out ${hovered ? "-translate-y-5" : "translate-y-10"} flex `}>
                    <div onClick={() => dispatch(getData(product))} className="p-1 bg-white rounded-sm cursor-pointer">
                    ...
                    <div className="p-1 bg-white rounded-sm cursor-pointer"  onClick={()=>{
                        setParam({category:category, id:product._id});
                            navigate(`/flower/${product.category}/${product._id}`)
                    }}
                >
                        <svg width={20} height={20} viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <span className="block mt-3 text-[1rem] text-linkColor">{product.title}</span>
                <span className="text-filterText font-bold text-logoColor">${product.price}</span>
            </div>
  )
}
